<script setup lang="ts">
    ...
    try{
		...
		if(import.meta.env.VITE_ENV == "production"){
			res = await axios.post("http://" + import.meta.env.VITE_PUBLIC_IP + "/api/pessoas", credentials);
		}else{
...
<template>
  ...
  >
    <div class="max-w-[400px] w-full flex flex-col items-center space-y-10">
      ...
      <p>Wants to see the data?<RouterLink to="/register" class="text-blue-300">Register</RouterLink> Or <RouterLink to="/register" class="text-blue-300">Log In</RouterLink></p>
    </div>
  </FormKit>
</template>
